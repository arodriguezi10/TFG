...
import { Plus, X, Lock, Crown, ChevronRight, Trash2 } from "lucide-react";
import {
...
const ConfigExerciseFreeMobile = () => {
    ...
    <div className="min-h-screen bg-background flex flex-col pb-20">
        ...
        <section className="mt-4 w-full px-4">
          <Card>
            <div className="flex flex-col items-center justify-center py-10 gap-4">
              ...
              <p className="font-body text-[14px] text-text-low text-center">Vuelve a la pantalla anterior y anade ejercicios.</p>
            </div>
          </Card>
        </section>
      ) : (
          ...
          <section key={exercise.id} className="mt-4 w-full px-4 flex flex-col gap-2.5">
            <Card>
              <div className="flex items-center gap-3.75">
                <p className="bg-primary-bg min-w-8.75 h-8.75 rounded-lg border border-primary font-heading font-bold text-[22px] text-primary text-center flex items-center justify-center px-2">{index + 1}</p>
                <div className="flex flex-col">
                  <p className="font-heading font-extrabold text-[18px] text-text-high">{exercise.name}</p>
                ...
                <div className="flex flex-col gap-1.25 ml-auto">
                  ...
                  <input
                    ...
                    className={`w-17.5 h-7.25 rounded-lg border font-heading font-bold text-[18px] text-center ${isInputFilled(exercisesRest[exercise.id]) ? "bg-accent2 border-accent2 text-background" : "bg-green-bg2 border-accent2 text-accent2"}`}
                    placeholder="1:30"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="grid grid-cols-[40px_1fr_1fr_1fr_40px] gap-2 items-center">
                  ...
                  <div className="flex items-center justify-center gap-0.5">
                    ...
                    {!hasProAccess && <Lock size={9} className="text-orange" />}
                  </div>
                  <div></div>
                </div>
                ...
                {(exercisesSeries[exercise.id] || []).map((serie, serieIndex) => (
                  <React.Fragment key={serie.id}>
                    <div className="grid grid-cols-[40px_1fr_1fr_1fr_40px] gap-2 items-center">
                      <p className="font-heading font-bold text-[22px] text-text-high text-center">S{serie.id}</p>
                      <input type="text" inputMode="numeric" value={serie.reps} onChange={e => updateSerie(exercise.id, serie.id, 'reps', e.target.value)}
                        className={`w-16.25 h-6.25 rounded-lg border font-body text-[12px] text-center ${isInputFilled(serie.reps) ? "bg-primary border-primary text-background" : "border-text-low text-text-high"}`} placeholder="12" />
                      <input type="text" inputMode="decimal" value={serie.weight} onChange={e => updateSerie(exercise.id, serie.id, 'weight', e.target.value)}
                        className={`w-16.25 h-6.25 rounded-lg border font-body text-[12px] text-center ${isInputFilled(serie.weight) ? "bg-primary border-primary text-background" : "border-text-low text-text-high"}`} placeholder="60" />
                      {hasProAccess ? (
                        <input type="text" inputMode="numeric" value={serie.rir || ""} onChange={e => updateSerie(exercise.id, serie.id, 'rir', e.target.value)}
                          className={`w-16.25 h-6.25 rounded-lg border font-body text-[12px] text-center ${isInputFilled(serie.rir) ? "bg-primary border-primary text-background" : "border-text-low text-text-high"}`} placeholder="2" />
                      ) : (
                        <span className="bg-orange-bg2 w-16.25 h-6.25 rounded-lg border border-orange flex items-center justify-center opacity-55 cursor-not-allowed"><Lock size={16} className="text-orange" /></span>
                      )}
                      <button onClick={() => deleteSerie(exercise.id, serie.id)} className="bg-surface w-6.25 h-6.25 rounded-lg border border-red flex items-center justify-center hover:bg-red/10 transition-colors"><X size={16} className="text-red" /></button>
                    </div>
              ...
              <div className="flex gap-2 mt-2 items-center">
                <p className="font-body text-[12px] text-text-low shrink-0">Tecnica</p>
                <div className="flex gap-1.5 overflow-x-auto scrollbar-hide">
                  {TECHNIQUES.map(technique => (
                    <button key={technique} onClick={() => toggleTechnique(exercise.id, technique)} disabled={!hasProAccess}
                      className={`px-3.5 py-1.5 rounded-[20px] border font-body text-[13px] font-medium whitespace-nowrap transition-all ${hasProAccess ? exercisesTechnique[exercise.id] === technique ? 'bg-primary border-primary text-background shadow-sm' : 'bg-transparent border-primary/40 text-primary hover:bg-primary/10 hover:border-primary' : 'bg-transparent border-text-low/30 text-text-low/50 cursor-not-allowed'}`}>
                      {technique}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between mt-2">
                <button onClick={() => addSerie(exercise.id)} className="font-body font-bold text-[14px] text-primary hover:text-primary/80 transition-colors flex items-center gap-1">
                  <Plus size={16} /> Anadir serie
                </button>
                <button onClick={() => handleRemoveExercise(exercise.id)} className="font-body font-semibold text-[14px] text-red hover:text-red/80 transition-colors flex items-center gap-1">
            ...
            <Card variant="outlined">
              <div className="flex items-center justify-between gap-3 cursor-pointer hover:bg-surface/50 transition-colors rounded-2xl -m-4 p-4">
                <div className="flex items-center gap-2.5">
                  <span className="bg-brown-bg h-13.75 w-13.75 px-4 rounded-xl border border-orange flex items-center justify-center"><Crown size={20} className="text-orange" /></span>
                  <div className="flex flex-col">
                    <p className="font-heading font-semibold text-[20px] text-text-high leading-tight text-left">Desbloquea RIR y tecnicas avanzadas</p>
                    <p className="font-body text-[12px] text-text-low text-left">Con <span className="text-orange">Plan Pro o Elite</span> activa RIR, Dropsets, Rest-pause y mas.</p>
                  </div>
                </div>
      ...
      {selectedExercises.length > 0 && (
        <section className="mt-4 w-full px-4 fixed bottom-1">
          <Button variant="outlined" text="Guardar rutina" bgColor="bg-primary" textColor="text-text-high" borderColor="border-primary" w="w-[100%]" onClick={handleSaveRoutine} />
        </section>
      )}
    </div>
  );
...
export default ConfigExerciseFreeMobile;