...
const DashboardDesktop = () => {
  ...
  const checkTodayHasProgressionRoutine = async (progressionData) => {
    ...
    const { data: calendar } = await supabase
      .from("progression_calendar").select("*")
      .eq("progression_id", progressionData.id).eq("date", todayDate)
      .eq("is_rest_day", false).single();
  ...
  const loadInitialData = async () => {
    try {
      ...
      const inRange = progressionData
        ...
        : false;
      await loadData(inRange && (await checkTodayHasProgressionRoutine(progressionData)));
    } catch (error) {
  ...
  const loadProgressionData = async () => {
    try {
      const { data: progressionData, error: progressionError } = await supabase
        .from("progressions").select("*").eq("user_id", user.id)
        .order("created_at", { ascending: false }).limit(1).single();

      if (progressionError || !progressionData) {
        setActiveProgression(null); setTodayProgressionRoutine(null); return null;
      }
      ...
      const { data: blocks } = await supabase
        ...
        .select(`*, routines (id, name, estimated_duration_min, routine_exercises (id, target_sets))`)
        .eq("progression_id", progressionData.id).order("position", { ascending: true });

      if (blocks) setProgressionBlocks(blocks);

      const { data: calendar } = await supabase
        .from("progression_calendar").select("*").eq("progression_id", progressionData.id);

      let assignments = {};
      if (calendar) {
        calendar.forEach((entry) => {
          assignments[entry.date] = { type: entry.is_rest_day ? "rest" : "routine", routineId: entry.routine_id };
        });
        ...
        if (todayAssignment && todayAssignment.type === "routine" && todayAssignment.routineId && blocks) {
          ...
          if (routineBlock) {
            ...
            setTodayProgressionRoutine(null); setRoutineCompletedToday(false);
          }
        } else {
          setTodayProgressionRoutine(null); setRoutineCompletedToday(false);
        }
      ...
      const { data: sessions } = await supabase
        .from("workout_sessions").select("session_date, routine_id").eq("user_id", user.id)
  ...
  const handleProgressionDayClick = (day) => {
    ...
    if (!assignment || assignment.type === "rest") { setSelectedProgressionDay(null); return; }
    if (selectedProgressionDay?.fullDate === day.fullDate) { setSelectedProgressionDay(null); return; }
    const routineBlock = progressionBlocks.find((b) => b.routine_id === assignment.routineId);
    if (routineBlock) {
      setSelectedProgressionDay({ ...day, routine: routineBlock.routines, color: routineBlock.color_code });
    }
  };
  ...
  const handleStartProgressionRoutine = () => {
    ...
    navigate(`/executeRoutine/${selectedProgressionDay.routine.id}`, {
      state: { fromProgression: true, progressionId: activeProgression.id, completedDate: selectedProgressionDay.fullDate },
    });
  };
  ...
  const checkIfRoutineCompletedToday = async (routineId) => {
    try {
      ...
      const { data, error } = await supabase.from("workout_sessions").select("id")
        .eq("user_id", user.id).eq("routine_id", routineId).eq("session_date", todayDate).limit(1);
  ...
  const loadWeekData = async () => {
    try {
      ...
      const { data: sessions } = await supabase.from("workout_sessions").select("*")
        .eq("user_id", user.id).gte("session_date", currentWeek[0].fullDate)
        .lte("session_date", currentWeek[6].fullDate).order("session_date", { ascending: true });
      setCompletedSessions(sessions || []);
    } catch (error) { console.error("Error cargando semana:", error); }
  };
  ...
  const handleDayClick = (day) => {
    ...
    if (session) { setSelectedDay(day); setSelectedSession(session); }
    else { setSelectedDay(null); setSelectedSession(null); }
  };
    ...
    try {
      ...
      if (!hasProgression) {
        const { data: routines, error } = await supabase.from("routines")
          .select(`*, routine_exercises (id, target_sets)`).eq("user_id", user.id)
          .order("created_at", { ascending: false });
        if (error) { setTodayRoutine(null); return; }
        const todayDayName = getTodayDayName();
        const routineForToday = routines?.find((routine) => {
          try { return JSON.parse(routine.assigned_days || "[]").includes(todayDayName); }
          catch { return false; }
        });
        ...
        if (routineForToday) {
          ...
          setRoutineCompletedToday(isCompleted);
        } else { setRoutineCompletedToday(false); }
      } else { setTodayRoutine(null); }
    } catch (error) { console.error("Error:", error); setTodayRoutine(null); }
  };

  const loadWeightData = async () => {
    try {
      const todayDate = getLocalDate();
      const { data: todayData } = await supabase.from("weight_logs").select("*")
        .eq("user_id", user.id).eq("log_date", todayDate).single();
      ...
      const { data: recentWeights } = await supabase.from("weight_logs").select("*")
        .eq("user_id", user.id).gte("log_date", thirtyDaysAgoDate).order("log_date", { ascending: false });
      ...
      const sevenDaysAgoDate = `${sevenDaysAgo.getFullYear()}-${String(sevenDaysAgo.getMonth() + 1).padStart(2, "0")}-${String(sevenDaysAgo.getDate()).padStart(2, "0")}`;
      const { data: last7DaysData } = await supabase.from("weight_logs").select("*")
        .eq("user_id", user.id).gte("log_date", sevenDaysAgoDate).order("log_date", { ascending: true });
    ...
    } catch (error) { console.error("Error cargando peso:", error); }
  };
  ...
  const handleSaveWeight = async () => {
    ...
    const todayDate = getLocalDate();
    try {
      const { data: existing } = await supabase.from("weight_logs").select("id")
        .eq("user_id", user.id).eq("log_date", todayDate).single();
      if (existing) {
        await supabase.from("weight_logs").update({ weight, updated_at: new Date().toISOString() }).eq("id", existing.id);
      } else {
        await supabase.from("weight_logs").insert({ user_id: user.id, weight, log_date: todayDate });
      }
      setShowWeightInput(false); setWeightInput("");
      await loadWeightData();
      alert("✅ Peso guardado correctamente");
    } catch (error) { console.error("Error guardando peso:", error); alert("❌ Error al guardar el peso"); }
  };
  ...
  const loadDailyChecks = () => {
    ...
    } else { resetDailyChecks(); }
  };

  const resetDailyChecks = () => {
    setPreWorkout(false); setPostWorkout(false);
    const today = new Date().toDateString();
  ...
  const handleStartRoutine = async () => {
    if (todayProgressionRoutine) {
      ...
      if (routineCompletedToday) {
        const { data: session } = await supabase.from("workout_sessions").select("id")
          .eq("user_id", user.id).eq("routine_id", todayProgressionRoutine.id).eq("session_date", todayDate).single();
      ...
      if (routineCompletedToday) {
        const todayDate = getLocalDate();
        const { data: session } = await supabase.from("workout_sessions").select("id")
          .eq("user_id", user.id).eq("routine_id", todayRoutine.id).eq("session_date", todayDate).single();
        navigate(`/executeRoutine/${todayRoutine.id}`, { state: { viewOnly: true, sessionId: session?.id } });
      } else { navigate(`/executeRoutine/${todayRoutine.id}`); }
    } else { navigate("/routines1"); }
  };
          ...
          return (
            <div key={index} className="flex flex-col items-center flex-1 gap-1">
              <div className="w-full flex items-end justify-center" style={{ height: "32px" }}>
                {day.weight ? (
                  <div className="w-full bg-accent1 rounded-t-sm" style={{ height: `${Math.max(height, 10)}%`, minHeight: "4px" }} />
                ) : (
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <p className="font-body text-[13px] text-text-low capitalize">{today}</p>
        ...
        <div className="grid grid-cols-5 gap-6">
          ...
          <div className="col-span-3 flex flex-col gap-6">
              ...
              {loading ? (
                ...
                <Card>
                  <div className="flex items-start justify-between mb-4">
                    <div>
                      <p className="font-heading font-extrabold text-[24px] text-text-high">{displayRoutine.name}</p>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                    {[
                      ...
                      <div key={stat.label} className="bg-surf rounded-xl p-3 text-center border border-text-low/20">
                        <p className="font-heading font-extrabold text-[24px] text-text-high">{stat.value}</p>
                        <p className="font-subheading font-bold text-[11px] text-text-low uppercase">{stat.label}</p>
                      </div>
                    ))}
                  </div>
                ...
                <Card>
                  <div className="flex items-center gap-6 py-4">
                    ...
                    <div className="flex-1">
                      <h3 className="font-heading font-extrabold text-[20px] text-text-high mb-1">Día de descanso</h3>
                      <p className="text-text-low text-[14px] font-light mb-3">No tienes rutinas programadas para hoy.</p>
                      <Button variant="outlined" text="Ver mis rutinas" bgColor="bg-surf" textColor="text-text-high" borderColor="border-text-low" w="w-48" onClick={handleStartRoutine} />
                    </div>
                  </div>
                </Card>
              )}
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <p className="font-subheading font-bold text-[12px] text-text-low uppercase tracking-wide">
                    Semana {currentProgressionWeek + 1} · {activeProgression.name}
                  </p>
                  <button onClick={() => navigate("/progression")} className="font-subheading font-bold text-primary text-[13px] hover:opacity-80">Ver todo →</button>
                </div>
                <Card>
                  <div className="flex items-center justify-between mb-4">
                    <p className="font-subheading font-bold text-[12px] text-text-low uppercase tracking-wide">{activeProgression.name}</p>
                    <div className="flex gap-2">
                      <button onClick={handlePreviousProgressionWeek} disabled={currentProgressionWeek === 0}
                        className={`h-7 w-7 rounded-lg border flex items-center justify-center text-[14px] ${currentProgressionWeek === 0 ? "opacity-30 cursor-not-allowed" : "hover:bg-surface"} bg-surf border-text-low text-text-high`}>←</button>
                      <button onClick={handleNextProgressionWeek} disabled={currentProgressionWeek >= activeProgression.duration_weeks - 1}
                        className={`h-7 w-7 rounded-lg border flex items-center justify-center text-[14px] ${currentProgressionWeek >= activeProgression.duration_weeks - 1 ? "opacity-30 cursor-not-allowed" : "hover:bg-surface"} bg-surf border-text-low text-text-high`}>→</button>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    {progressionWeekDays.map((day, index) => {
                      ...
                      return (
                        <button key={index} onClick={() => handleProgressionDayClick(day)} className="flex flex-col items-center gap-1.5">
                          <p className="font-subheading font-bold text-[12px] text-text-low">{day.dayName}</p>
                          <div className={`h-10 w-10 rounded-xl font-heading font-bold text-[15px] flex items-center justify-center transition-all ${day.isToday ? "border-2 border-accent1 bg-accent1/10 text-accent1" : selectedProgressionDay?.fullDate === day.fullDate ? "border-2 border-primary bg-primary/10 text-primary" : isCompleted ? "border-2 border-accent3 bg-accent3/10 text-accent3" : "border border-text-low bg-surf text-text-high"}`}
                          ...
                          <p className="text-[12px]">{isCompleted ? "✅" : isRest ? "😴" : routineId ? "💪" : "·"}</p>
                        </button>
                      );
                    })}
                  </div>
                  {selectedProgressionDay && (
                    <div className="mt-4 pt-4 border-t border-text-low/20">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-3">
                          <div className="h-9 w-9 rounded-xl flex items-center justify-center text-[16px]" style={{ backgroundColor: `${selectedProgressionDay.color}20`, border: `1px solid ${selectedProgressionDay.color}` }}>💪</div>
                          <div>
                            <p className="font-body text-[11px] text-text-low">{selectedProgressionDay.dayName} {selectedProgressionDay.dayNum}</p>
                            <p className="font-heading font-bold text-[15px] text-text-high">{selectedProgressionDay.routine.name}</p>
                          </div>
                        </div>
                        <button onClick={() => setSelectedProgressionDay(null)} className="h-7 w-7 rounded-lg border border-text-low flex items-center justify-center text-text-low text-[12px]">✕</button>
                      </div>
                      {progressionCompletedDays.has(selectedProgressionDay.fullDate) ? (
                        <div className="bg-accent3/10 border border-accent3 rounded-xl p-3 flex items-center gap-3">
                          ...
                          <p className="font-heading font-bold text-[14px] text-accent3">Rutina completada</p>
                        </div>
                      ) : selectedProgressionDay.isToday ? (
                        <Button variant="outlined" text="⚡ Iniciar" bgColor="bg-accent1" textColor="text-text-high" borderColor="border-accent1" w="w-full" onClick={handleStartProgressionRoutine} />
                      ) : (
                        <div className="bg-surf/50 border border-text-low rounded-xl p-3 flex items-center gap-3">
                          <span className="text-[20px]">🔒</span>
                          <p className="font-body text-[13px] text-text-low">Solo disponible el día indicado</p>
                        </div>
                      )}
                    </div>
                  )}
            ...
            {/* HISTORIAL SEMANAL */}
            <div>
              <p className="font-subheading font-bold text-[12px] text-text-low uppercase tracking-wide mb-3">Historial semanal</p>
              <Card>
                ...
                <div className="flex items-center justify-between">
                  {weekDays.map((day, index) => {
                    ...
                    return (
                      <button key={index} onClick={() => handleDayClick(day)} disabled={!isCompleted}
                        className={`flex flex-col items-center gap-1.5 ${isCompleted ? "cursor-pointer" : "cursor-default"}`}>
                        <p className="font-subheading font-bold text-text-low text-[13px]">{day.dayStr}</p>
                        <div className={`h-10 w-10 rounded-xl font-heading font-bold text-text-high text-[15px] flex items-center justify-center transition-all ${isCompleted ? "bg-accent1" : day.isToday ? "border-2 border-accent1 text-accent1" : "text-text-low"}`}>
                          {day.dayNum}
                        </div>
                        <p className="text-[13px] text-text-low">{isCompleted ? "✓" : "·"}</p>
                      </button>
                    );
                  })}
                </div>
                {selectedDay && selectedSession && (
                  <div className="mt-4 pt-4 border-t border-text-low/20">
                    <p className="font-heading font-bold text-text-high text-[16px] mb-2">{selectedSession.routine_name}</p>
                    <div className="flex gap-4 text-[13px] text-text-low">
          ...
          <div className="col-span-2 flex flex-col gap-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <p className="font-subheading font-bold text-[12px] text-text-low uppercase tracking-wide">Peso de hoy</p>
                <button onClick={() => navigate("/progress")} className="font-subheading font-bold text-primary text-[12px] hover:opacity-80">Historial →</button>
              </div>
              <Card>
                <div className="flex items-center justify-between mb-3">
                  <p className="font-subheading font-bold text-text-low text-[12px] uppercase">Peso del día</p>
                ...
                {showWeightInput && (
                  <div className="mb-3 flex gap-2">
                    <input type="number" step="0.1" placeholder="75,5" value={weightInput} onChange={(e) => setWeightInput(e.target.value)}
                      className="flex-1 min-w-0 bg-background border border-text-low rounded-lg px-3 py-2 text-text-high text-[15px] font-heading font-semibold outline-none focus:border-accent1" autoFocus />
                    <button onClick={handleSaveWeight} className="shrink-0 bg-accent1 text-text-high px-3 py-2 rounded-lg font-heading font-bold text-[13px] hover:opacity-80">OK</button>
                  </div>
                )}
                {todayWeight ? (
                  <>
                    <div className="flex items-center justify-between">
                      <div className="flex items-baseline gap-1">
                        <span className="font-heading font-extrabold text-[48px] text-text-high leading-none">{formatWeight(todayWeight.weight).integer}</span>
                        <span className="font-heading font-bold text-[24px] text-text-low leading-none">,{formatWeight(todayWeight.weight).decimal}</span>
                        <span className="font-heading font-bold text-[18px] text-text-low ml-1">kg</span>
                      </div>
                      {monthlyChange !== null && (
                        <div className="bg-surf rounded-xl py-2 px-3 border border-accent2">
                          <p className="font-heading font-extrabold text-accent2 text-[18px] leading-tight">{monthlyChange > 0 ? "+" : ""}{monthlyChange.toFixed(1)}</p>
                          <p className="font-subheading font-semibold text-text-low text-[10px]">kg/mes</p>
                        </div>
                      )}
                    </div>
                    {weeklyChange !== null && (
                  ...
                  <div className="flex flex-col items-center justify-center py-6 text-center">
                    ...
                    <p className="text-text-low text-[13px] mb-3">No has registrado tu peso hoy</p>
                    <button onClick={() => setShowWeightInput(true)} className="bg-accent1 text-text-high px-4 py-2 rounded-xl font-heading font-bold text-[13px] hover:opacity-80">
                      Registrar peso
                    </button>
                  </div>
                )}
            ...
            {/* PRE / POST ENTRENO */}
            <div>
              <p className="font-subheading font-bold text-[12px] text-text-low uppercase tracking-wide mb-3">¿Todo marcado hoy?</p>
              <div className="flex flex-col gap-3">
                <button onClick={handlePreWorkoutToggle} className="w-full cursor-pointer">
                  <Card>
                    <div className="flex items-center gap-4">
                      <div className={`h-10 w-10 rounded-xl border flex justify-center items-center text-[18px] transition-colors shrink-0 ${preWorkout ? "bg-accent1 border-accent1" : "bg-surf border-text-low"}`}>
                        {preWorkout ? "✓" : "○"}
                      </div>
                      <div className="text-left">
                        <p className={`font-subheading font-bold text-[15px] transition-colors ${preWorkout ? "text-accent1" : "text-text-high"}`}>Pre-entreno</p>
                        <p className="font-body text-[12px] text-text-low">Carbohidratos</p>
                      </div>
                    </div>
                  </Card>
                </button>
                <button onClick={handlePostWorkoutToggle} className="w-full cursor-pointer">
                  <Card>
                    <div className="flex items-center gap-4">
                      <div className={`h-10 w-10 rounded-xl border flex justify-center items-center text-[18px] transition-colors shrink-0 ${postWorkout ? "bg-accent1 border-accent1" : "bg-surf border-text-low"}`}>
                        {postWorkout ? "✓" : "○"}
                      </div>
                      <div className="text-left">
                        <p className={`font-subheading font-bold text-[15px] transition-colors ${postWorkout ? "text-accent1" : "text-text-high"}`}>Post-entreno</p>
                        <p className="font-body text-[12px] text-text-low">Proteína</p>
                      </div>
                    </div>
                  </Card>
                </button>
