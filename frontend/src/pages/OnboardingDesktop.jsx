...
const OnboardingDesktop = () => {
  ...
  const validateForm = () => {
    ...
    if (
      ...
      newErrors.height_cm = "Altura valida (100-250 cm)";
    if (
      !formData.initial_weight_kg ||
      formData.initial_weight_kg < 30 ||
      formData.initial_weight_kg > 300
    )
      newErrors.initial_weight_kg = "Peso valido (30-300 kg)";
    if (!formData.bio || formData.bio.trim().length < 10)
      newErrors.bio = "Al menos 10 caracteres";
    if (!formData.fitness_goal)
      newErrors.fitness_goal = "Selecciona tu objetivo";
    if (!formData.activity_level)
      newErrors.activity_level = "Selecciona tu nivel";
    setErrors(newErrors);
    ...
    return (
      <div className="min-h-screen bg-background flex">
        ...
        <div className="w-1/2 bg-surf border-r border-text-low/20 flex flex-col items-center justify-center p-12">
          <div className="bg-accent1 h-20 w-20 rounded-3xl flex items-center justify-center font-heading font-extrabold text-[40px] text-text-high mb-6">
            F
          </div>
          <h1 className="font-heading font-extrabold text-[40px] text-text-high mb-3 text-center">
            Bienvenido a FYLIOS
          </h1>
          <p className="font-body text-[16px] text-text-low text-center max-w-sm">
            La plataforma de entrenamiento que conecta atletas y entrenadores
          </p>
        ...
        <div className="w-1/2 flex flex-col items-center justify-center p-12">
          <div className="w-full max-w-md">
            <h2 className="font-heading font-extrabold text-[28px] text-text-high mb-2">
              Como vas a usar FYLIOS?
            </h2>
            <p className="font-body text-[14px] text-text-low mb-8">
            ...
            <div className="flex flex-col gap-4 mb-8">
              ...
              >
                <div
                  className={`h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 ${role === "athlete" ? "bg-primary" : "bg-background border border-text-low"}`}
                >
                  <Dumbbell
                    size={26}
                    className={
              ...
              >
                <div
                  className={`h-14 w-14 rounded-2xl flex items-center justify-center shrink-0 ${role === "coach" ? "bg-orange" : "bg-background border border-text-low"}`}
                >
                  <Users
                    size={26}
                    className={
                ...
                {role === "coach" && (
                  <CheckCircle2
                    size={20}
                    className="text-orange ml-auto shrink-0"
                  />
                )}
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      <div className="w-1/2 bg-surf border-r border-text-low/20 flex flex-col items-center justify-center p-12">
        ...
        <h2 className="font-heading font-extrabold text-[32px] text-text-high mb-3 text-center">
          Perfil de atleta
        </h2>
        <p className="font-body text-[15px] text-text-low text-center max-w-sm mb-10">
          Estos datos nos permiten personalizar tu experiencia y darte
        ...
        <div className="flex flex-col gap-4 w-full max-w-sm">
          {[
            {
              title: "Calculos precisos",
              desc: "Usamos tu altura y peso para calcular volumenes y cargas optimas",
            },
            {
              title: "Objetivos personalizados",
              desc: "Tu goal define como se estructuran tus entrenamientos",
            },
            {
              title: "Seguimiento real",
              desc: "Comparamos tu progreso con tu punto de partida",
            },
          ].map(({ title, desc }) => (
            <div key={title} className="flex items-start gap-3">
              ...
              />
              <div>
                <p className="font-subheading font-bold text-[14px] text-text-high">
                  {title}
                </p>
      ...
      <div className="w-1/2 overflow-y-auto flex flex-col items-center justify-center p-12">
        <div className="w-full max-w-md">
          <h2 className="font-heading font-extrabold text-[28px] text-text-high mb-6">
            Completa tu perfil
          </h2>

          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            <div>
              <label className="font-subheading font-bold text-[13px] text-text-low uppercase tracking-wide block mb-2">
                Sexo
              </label>
