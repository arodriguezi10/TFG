...
const MedidaCard = ({ icon, label, value, unit, color, borderColor, bgColor }) => (
  <div className="flex-1 rounded-xl p-3 border flex flex-col gap-1" style={{ backgroundColor: bgColor, borderColor }}>
    ...
    <div className="flex items-baseline gap-0.5">
      <p className="font-heading font-extrabold text-[20px] leading-none" style={{ color }}>{value ?? "--"}</p>
...
const ProgressCuerpoMobile = ({ subscriptionTier }) => {
  ...
  const renderWeightChart = () => {
    ...
    const width = 340, height = 160, paddingLeft = 36, paddingRight = 12, paddingTop = 16, paddingBottom = 28;
    ...
    return (
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
        ...
        <circle cx={getX(validPoints.length - 1)} cy={getY(validPoints[validPoints.length - 1].weight_kg)} r="4" fill="#ff6b9d" stroke="#0a0a0f" strokeWidth="2" />
        {yLabels.map((val, i) => <text key={i} x={paddingLeft - 4} y={getY(val) + 4} textAnchor="end" fontSize="10" fill="#6b6b8a">{val}</text>)}
        {xLabelIndices.map((idx) => <text key={idx} x={getX(idx)} y={height - 4} textAnchor="middle" fontSize="10" fill="#6b6b8a">{idx === validPoints.length - 1 ? "Hoy" : formatAxisDate(validPoints[idx].created_at)}</text>)}
      </svg>
    );
  };
  ...
  return (
    <div className="flex flex-col px-4 gap-4">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => navigate("/dailyRegister")} className="font-subheading font-bold text-[14px] text-primary">+ Registrar semana</button>
      </div>
        ...
        {RANGES.map(({ label, minTier }) => {
          ...
          return (
            <button key={label} onClick={() => handleRangeChange(label, minTier)}
              className={`px-4 py-1.5 rounded-xl font-subheading font-bold text-[13px] border transition-all ${selectedRange === label && !locked ? "bg-accent2 border-accent2 text-text-high" : "bg-surf border-text-low text-text-low"}`}>
              {locked ? <span className="flex flex-col items-center leading-tight"><Lock size={9} /><span>{label}</span></span> : label}
            </button>
          );
        })}
      </div>
      ...
      {latestMeasurement?.weight_kg ? (
        <div className="flex items-center gap-3">
          <span className="font-heading font-extrabold text-[48px] text-text-high leading-none">{latestMeasurement.weight_kg}</span>
      ...
      <p className="font-body text-[12px] text-text-low -mt-2">Peso corporal · Ultimas {getRangeLabel(selectedRange)}</p>

      <Card>{renderWeightChart()}</Card>

      <p className="font-subheading font-bold text-[13px] text-text-low uppercase tracking-wide mt-2">Medidas esta semana</p>
      ...
      {weeklyCheckin ? (
        <>
          <div className="flex gap-3">
            <Card>
              <div className="flex flex-col gap-1">
                <div className="h-9 w-9 rounded-lg bg-red-bg1 border border-red flex items-center justify-center shrink-0"><Percent size={16} className="text-red" /></div>
            ...
            <Card>
              <div className="flex flex-col gap-1">
                <div className="h-9 w-9 rounded-lg bg-accent2/10 border border-accent2 flex items-center justify-center shrink-0"><Ruler size={16} className="text-accent2" /></div>
          ...
          <div className="flex gap-2">
            <MedidaCard icon={<Dumbbell size={14} color="#6c63ff" />} label="Pecho" value={weeklyCheckin.chest_cm} unit="cm" color="#6c63ff" borderColor="rgba(108,99,255,0.4)" bgColor="rgba(108,99,255,0.06)" />
            <MedidaCard icon={<Scale size={14} color="#f5a623" />} label="Brazo" value={weeklyCheckin.arm_cm} unit="cm" color="#f5a623" borderColor="rgba(245,166,35,0.4)" bgColor="rgba(245,166,35,0.06)" />
            <MedidaCard icon={<PersonStanding size={14} color="#ff6b9d" />} label="Pierna" value={weeklyCheckin.leg_cm} unit="cm" color="#ff6b9d" borderColor="rgba(255,107,157,0.4)" bgColor="rgba(255,107,157,0.06)" />
          </div>
        </>
      ) : (
        <button onClick={() => navigate("/dailyRegister")} className="w-full bg-surf border border-text-low border-dashed rounded-2xl py-6 flex flex-col items-center gap-2">
      ...
      <div className="flex items-center justify-between mt-2">
        <p className="font-subheading font-bold text-[14px] text-text-high tracking-wide">Sensaciones y Recuperacion</p>
      ...
      <Card>
        <div className="flex flex-col divide-y divide-text-low/20">
          {[
            ...
            <div key={key} className="py-3 first:pt-0 last:pb-0">
              <div className="flex items-center gap-3 mb-2">
                <div className={`h-9 w-9 rounded-lg ${bg} border ${border} flex items-center justify-center shrink-0`}>{icon}</div>
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <p className="font-subheading font-bold text-[13px] text-text-low uppercase tracking-wide">{label}</p>
                    <p className="font-body text-[11px] text-text-low">Esta semana</p>
                  </div>
                  <div className="flex items-baseline gap-1 mt-0.5">
                    <p className="font-heading font-extrabold text-[22px] text-text-high leading-none">{weeklyCheckin?.[key] ?? "--"}</p>
          ...
          <div className="py-3">
            <div className="flex items-center gap-3 mb-2">
              <div className="h-9 w-9 rounded-lg bg-accent2/10 border border-accent2 flex items-center justify-center shrink-0"><Wind size={16} className="text-accent2" /></div>
            ...
            <div className="flex gap-2">
              {["Ninguna", "Baja", "Media", "Alta"].map((nivel) => (
                <span key={nivel} className="px-2.5 py-1 rounded-full font-subheading font-bold text-[12px] border transition-all"
          ...
          <div className="py-3">
            <div className="flex items-center gap-3 mb-2">
              <div className="h-9 w-9 rounded-lg bg-accent1/10 border border-accent1 flex items-center justify-center shrink-0"><Bone size={16} className="text-accent1" /></div>
      ...
      <button onClick={() => navigate("/dailyRegister")} className="w-full bg-surf border border-text-low rounded-2xl px-4 py-4 flex items-center gap-4 hover:border-primary transition-colors">
        <div className="h-10 w-10 rounded-xl bg-primary-bg border border-primary flex items-center justify-center shrink-0"><ClipboardList size={18} className="text-primary" /></div>
        <div className="flex-1 text-left">
          <p className="font-heading font-bold text-[16px] text-text-high">{weeklyCheckin ? "Actualizar registro semanal" : "Registrar sensaciones de la semana"}</p>
          <p className="font-body text-[12px] text-text-low">{weeklyCheckin ? "Modifica tu check-in de esta semana" : "Envia tu check-in semanal al entrenador"}</p>
        </div>
...
export default ProgressCuerpoMobile;