...
const ProgressVolumenMobile = ({ subscriptionTier }) => {
  ...
  if (!isElite) return (
    <div className="flex flex-col px-4 gap-6 py-8">
      ...
      <div className="text-center">
        <h2 className="font-heading font-extrabold text-[26px] text-text-high leading-tight mb-2">Analisis de<br /><span className="text-primary">Volumen Efectivo</span></h2>
        <p className="font-body text-[14px] text-text-low leading-relaxed">Controla tu volumen semanal por grupo muscular basado en los rangos MEV y MRV.</p>
      </div>
      <div className="flex flex-col gap-3">
        {[
          { icon: <Activity size={18} className="text-primary" />, title: "MEV y MRV por musculo", desc: "Saber exactamente cuantas series necesitas para crecer" },
          { icon: <TrendingUp size={18} className="text-accent3" />, title: "Tendencia semanal", desc: "Compara tu volumen con la semana anterior" },
          { icon: <AlertTriangle size={18} className="text-orange-400" />, title: "Analisis de fatiga", desc: "Detecta sobreentrenamiento antes de que ocurra" },
          { icon: <CheckCircle2 size={18} className="text-accent3" />, title: "Proyeccion de recuperacion", desc: "Dias estimados hasta recuperacion completa por musculo" },
        ].map(f => (
          <div key={f.title} className="flex items-start gap-4 bg-surf border border-text-low/20 rounded-2xl p-4">
            <div className="h-10 w-10 rounded-xl bg-background flex items-center justify-center shrink-0">{f.icon}</div>
            <div>
      ...
      <button onClick={() => navigate("/subscription")} className="w-full bg-primary border border-primary rounded-2xl py-4 font-heading font-bold text-[16px] text-text-high hover:opacity-90 transition-opacity flex items-center justify-center gap-2">
  ...
  return (
    <div className="flex flex-col px-4 gap-4">
      ...
      <p className="font-body text-[12px] text-text-low -mt-2">de Volumen Efectivo</p>
      ...
      {muscleData.length === 0 ? (
        <Card>
          <div className="flex flex-col items-center py-10 gap-3">
            <BarChart2 size={40} className="text-text-low" />
            <p className="font-heading font-bold text-[16px] text-text-high">Sin datos esta semana</p>
            <p className="font-body text-[13px] text-text-low text-center">Completa entrenamientos para ver tu analisis de volumen</p>
          </div>
        </Card>
      ) : (
        muscleData.map(item => {
            ...
            <div key={item.muscle} className="rounded-2xl p-4 border" style={{ backgroundColor: config.bg, borderColor: config.border }}>
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="flex-1">
                  <p className="font-heading font-bold text-[18px] text-text-high">{item.muscle}</p>
              ...
              <div className="flex items-center gap-3 mb-3">
                <p className="font-body text-[13px] text-text-low">{item.series} series · Fatiga:</p>
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full font-subheading font-bold text-[12px]" style={{ border: `1px solid ${config.color}`, color: config.color }}>
                  {config.fatiga}
              ...
              <div className="w-full h-2 bg-background/40 rounded-full overflow-hidden mb-2">
                <div className="h-full rounded-full transition-all" style={{ width: `${item.progressPct}%`, backgroundColor: config.color }} />
              </div>
      ...
      {muscleData.length > 0 && (
        <>
          <div className="flex items-center gap-2 mt-2">
            <p className="font-subheading font-bold text-[13px] text-text-low uppercase tracking-wide">Analisis de fatiga y recuperacion</p>
                ...
                return (
                  <div key={item.muscle} className="py-4 first:pt-0 last:pb-0">
                    <div className="flex items-start gap-3 mb-2">
                      <div className="h-9 w-9 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: analysis.iconBg, border: `1px solid ${analysis.iconBorder}` }}>
                        {analysis.icon}
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="font-body text-[12px] text-text-low leading-relaxed">{analysis.descripcion}</p>
                      </div>
                    </div>
                    <span className="inline-flex px-3 py-1 rounded-full font-subheading font-bold text-[12px] border" style={{ color: analysis.recomendacionColor, borderColor: analysis.recomendacionColor, backgroundColor: `${analysis.recomendacionColor}10` }}>
          ...
          <div className="rounded-2xl p-4 border" style={{ backgroundColor: "rgba(245,166,35,0.05)", borderColor: "#f5a623" }}>
            <p className="font-heading font-bold text-[16px] text-text-high mb-1">Proyeccion de recuperacion</p>
            <p className="font-body text-[12px] text-text-low mb-4">Dias estimados hasta recuperacion completa</p>
            <div className="grid grid-cols-3 gap-2">
              {muscleData.map(item => {
                const recovery = getRecoveryDays(item.status);
                return (
                  <div key={item.muscle} className="bg-surf rounded-xl p-2.5 flex flex-col gap-0.5">
                    <p className="font-body text-[10px] text-text-low">{item.muscle}</p>
                    <p className="font-heading font-extrabold text-[22px] leading-none" style={{ color: recovery.color }}>{recovery.days}</p>
...
export default ProgressVolumenMobile;