...
const WorkoutSummaryMobile= () => {
    ...
    Object.values(exercisesData).forEach((exerciseSeries) => {
      exerciseSeries.forEach((serie) => {
        if (serie.completed && serie.actualReps && serie.actualWeight) {
          totalVolume +=
            parseFloat(serie.actualReps) * parseFloat(serie.actualWeight);
        }
      });
    });
  ...
  const calculateExerciseStats = () => {
    ...
    Object.keys(exercisesData).forEach((exerciseId) => {
      ...
      const percentage =
        totalSeries > 0 ? (completedSeries.length / totalSeries) * 100 : 0;
      ...
      if (percentage < 100 && percentage >= 80) {
        const incompleteSerie =
          exerciseSeries.findIndex((s) => !s.completed) + 1;
          ...
          {exerciseStats.map((exercise, index) => {
            ...
            return (
              <Card key={index}>
                ...
                >
                  <div
                    className={`h-11 w-11 rounded-lg ${exercise.iconBg} border ${exercise.borderColor} flex items-center justify-center text-[20px] shrink-0`}
                  >
                    {exercise.icon}
                  </div>
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <p
                      className={`font-body text-[11px] ${exercise.statusColor} mt-0.5`}
                    >
                      {exercise.status}
                    </p>
                  </div>
                  ...
                  <div
                    className={`text-text-low text-[16px] transition-transform ${isExpanded ? "rotate-180" : ""}`}
                  >
                    ⌄
                  </div>
                </button>
                    ...
                    {exercise.series.map((serie, idx) => (
                      ...
                      >
                        ...
                        <div className="font-heading font-semibold text-[13px] text-accent3">
                          {serie.completed
                            ? `${serie.actualReps}×12@RIR2`
                            : "—"}
                        </div>
                        <div
                          className={`font-heading font-bold text-[14px] px-2 py-1 rounded-lg ${
                            serie.completed
                        ...
                        >
                          {serie.completed
                            ? "100%"
                            : `${Math.round(((parseInt(serie.actualReps) || 0) / serie.targetReps) * 100)}%`}
                        </div>
                      </div>
                    ))}
...
export default WorkoutSummaryMobile;