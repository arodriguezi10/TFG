...
const OnboardingMobile = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.height_cm || formData.height_cm < 100 || formData.height_cm > 250) newErrors.height_cm = "Introduce una altura valida (100-250 cm)";
    if (!formData.initial_weight_kg || formData.initial_weight_kg < 30 || formData.initial_weight_kg > 300) newErrors.initial_weight_kg = "Introduce un peso valido (30-300 kg)";
    if (!formData.bio || formData.bio.trim().length < 10) newErrors.bio = "Escribe al menos 10 caracteres sobre ti";
      ...
      <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-md">
          <div className="text-center mb-8">
            <div className="bg-accent1 h-16 w-16 rounded-2xl flex items-center justify-center font-heading font-extrabold text-[32px] text-text-high mx-auto mb-4">F</div>
            <h1 className="font-heading font-extrabold text-[28px] text-text-high mb-2">Bienvenido a FYLIOS</h1>
            <p className="font-body text-[14px] text-text-low">Cuéntanos cómo vas a usar la app</p>
          </div>
          ...
          <div className="flex flex-col gap-3 mb-6">
            <button onClick={() => setRole("athlete")}
              ...
              <div className={`h-12 w-12 rounded-xl flex items-center justify-center shrink-0 ${role === "athlete" ? "bg-primary" : "bg-background border border-text-low"}`}>
                <Dumbbell size={22} className={role === "athlete" ? "text-text-high" : "text-text-low"} />
              </div>
              <div>
                <p className="font-heading font-bold text-[18px] text-text-high">Soy atleta</p>
                <p className="font-body text-[13px] text-text-low">Quiero entrenar, registrar mi progreso y mejorar</p>
              </div>
            </button>
            ...
            <button onClick={() => setRole("coach")}
              ...
              <div className={`h-12 w-12 rounded-xl flex items-center justify-center shrink-0 ${role === "coach" ? "bg-orange" : "bg-background border border-text-low"}`}>
                <Users size={22} className={role === "coach" ? "text-text-high" : "text-text-low"} />
              </div>
              <div>
                <p className="font-heading font-bold text-[18px] text-text-high">Soy entrenador</p>
                <p className="font-body text-[13px] text-text-low">Quiero supervisar y gestionar a mis atletas</p>
              </div>
            </button>
          </div>
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          <h1 className="font-heading font-extrabold text-[28px] text-text-high mb-2">Completa tu perfil</h1>
          <p className="font-body text-[14px] text-text-low">Necesitamos estos datos para personalizar tu experiencia</p>
        </div>

        <Card>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="font-subheading font-bold text-[14px] text-text-high mb-2 block">Sexo</label>
            ...
            <div>
              <label className="font-subheading font-bold text-[14px] text-text-high mb-2 block">Altura (cm)</label>
            ...
            <div>
              <label className="font-subheading font-bold text-[14px] text-text-high mb-2 block">Peso inicial (kg)</label>
            ...
            <div>
              <label className="font-subheading font-bold text-[14px] text-text-high mb-2 block">Objetivo principal</label>
            ...
            <div>
              <label className="font-subheading font-bold text-[14px] text-text-high mb-2 block">Nivel de actividad fisica</label>
            ...
            <div>
              <label className="font-subheading font-bold text-[14px] text-text-high mb-2 block">Cuentanos sobre ti</label>
