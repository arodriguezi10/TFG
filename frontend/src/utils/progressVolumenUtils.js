...
export const getRecoveryDays = (status) => {
  switch (status) {
    case "mrv_excedido":
      return { days: "4-5d", label: "Alta fatiga", color: "#ff5757" };
    case "cerca_mrv":
      return { days: "2-3d", label: "Precaucion", color: "#f5a623" };
    case "mav_optimo":
      return { days: "2d", label: "En progreso", color: "#6c63ff" };
    default:
      return { days: "1d", label: "Sin fatiga", color: "#36d9b8" };
  }
};
  ...
  switch (status) {
    ...
    default:
      return {
        ...
        recomendacion: `Anade +${mev - series} series prox. sem.`,
...
export const loadVolumeData = async (userId, weekOffset, setMuscleData, setWeekOptions, setLoading) => {
  ...
  try {
    ...
    const { monday: prevMonday, sunday: prevSunday } = getWeekRange(
      weekOffset - 1,
    );
    ...
    (exerciseList || []).forEach((e) => {
      exerciseMap[e.id] = e.muscle_group;
    });
